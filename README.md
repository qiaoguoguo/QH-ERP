...
当前稳定交付已经完成从 `000` 到 `027` 的业务阶段，并完成 025A、027A 两轮页面与现存问题治理。`024 采购深化与项目专采` 已交付项目/公共采购来源，`025 销售深化与项目履约` 已交付可信有效销售需求，`026 订单驱动缺料与供给建议` 已交付可追溯建议，`027 项目生产执行与外协基础` 已将已确认生产/外协建议幂等承接为项目工单或外协订单，并闭合项目归属下的领退补料、报工、完工、外协发料/收货和追溯；027A 已按 `docs/ui/page-standards.md` 完成受影响页面逐页治理和已登记问题闭环。

后续 `028` 至 `036` 继续按“项目型制造的业财成本闭环优先”推进。下一阶段 `028 发票、费用与往来深化` 已完成固定五角色唯一目标讨论并冻结权威阶段说明，业务实现尚未开始；路线规划和阶段说明不得表达为功能已经交付。

正式验收环境沿用受控电气制造演示数据，不因 024 至 027 前向迁移重建既有事实；027 正式部署后数据库版本为 Flyway V29，独立验证规则精确要求 V29。MinIO 与数据库可用文件对象保持两侧相等且不少于 8，当前交付基线为 18/18；对象数允许随合法附件、导入、导出或打印任务增长。运行入口、测试隔离和验收细节以当前交接为准。
...
- 第一版以销售项目为一级成本和利润对象；项目合同、库存计价、项目专采、销售履约、订单驱动供给和项目生产/外协执行已在 020、023、024、025、026、027 完成，后续优先补齐发票、费用、正式项目成本、月结和正式财务。
