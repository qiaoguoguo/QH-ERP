...
当前已经正式交付从 `000` 到 `031` 的业务阶段，并完成 025A、027A 两轮页面与现存问题治理。`024` 至 `027` 已形成项目采购、销售履约、订单驱动供给和项目生产/外协可信来源；`028 发票、费用与往来深化` 已交付业务结算来源；`029 项目成本核算` 已交付项目成本子账；`030 业务月结与期间快照` 已交付月末检查、原子关闭/重开、不可变多版本快照和八类固定经营报表基线；`031 会计科目、凭证与总账基础` 已正式交付单公司、单账簿、人民币的会计期间、科目、辅助核算、凭证、审批记账、总账、明细账、科目余额和试算平衡基础。

后续 `032` 至 `036` 继续按“项目型制造的业财成本闭环优先”推进。下一业务阶段为 `032 财务结账与资金税务基础`；路线规划不得提前表达为功能已经交付。

正式验收环境沿用受控电气制造演示数据，不因 024 至 031 前向迁移重建既有事实。当前正式环境已由 Flyway V32 前迁至 V33；V29/V30/V31/V32 checksum 分别为 `774334682`、`2130342893`、`-2074547591`、`249406902`，V33 checksum 为 `612501943`，失败迁移 0。031 隔离验证历史证据为 140/140（138 SQL + 2 环境）；正式完整验证器为 142/142、failed 0、数据库/MinIO 对象 18/18，GL 门禁为 `activeRules=7`、`activeLines=17`、`activeAuxMaps=9` 且违反数为 0；正式 `gl_voucher`、`posted`、`ledger`、`sourceClaims` 均为 0，未补造历史总账或验收事实。运行入口、测试隔离和验收细节以当前交接为准。
...
- 第一版以销售项目为一级成本和利润对象；项目合同、库存计价、项目专采、销售履约、订单驱动供给、项目生产/外协执行、业务发票费用往来及项目成本子账已在 020、023 至 029 完成，030 已补齐业务月结和期间快照，031 已正式补齐会计科目、凭证与总账基础，后续进入 032 财务结账与资金税务基础。
