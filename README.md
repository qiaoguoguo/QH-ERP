...
当前任务分支已完成从 `000` 到 `018` 的基础阶段，并完成页面基础体验治理。`018 库存可用量、占用、冻结与预留` 已提交为本地基线，建立了账面库存、合格现存、占用库存、预留库存、冻结库存、采购在途参考、现货净可用、可承诺量和净需求缺口的统一口径。

当前进入 `019 批次、序列号与来源去向追溯基础`，分支为 `codex/019-batch-serial-trace`。019 将在 017 质量状态和 018 可用量底座之上补齐物料追踪方式、批次/序列库存底账、核心单据追踪分配、质量冻结联动和来源去向追溯。后续阶段推进必须基于最新交接文档、当前仓库状态和用户最新指令，复用产品经理、UI 设计师、前端开发、后端开发、测试五个固定角色子代理。主代理只负责总体规划、范围控制、并行协调、审查汇总和交付判断，不因执行耗时、故障或返工压力直接接手业务实现。
...
- [仓库规范](AGENTS.md)
- [文档索引](docs/README.md)
- [当前节点交接报告](docs/handoffs/2026-07-06-project-handoff-current.md)
