...
当前任务分支已完成从 `000` 到 `018` 的基础阶段，并完成页面基础体验治理。`018 库存可用量、占用、冻结与预留` 在库存质量状态基础上建立了账面库存、合格现存、占用库存、预留库存、冻结库存、采购在途参考、现货净可用、可承诺量和净需求缺口的统一口径，为销售可承诺量与计划净需求提供可信底座。

018 当前仍在任务分支 `codex/018-inventory-availability-reservation` 中，尚未提交、合入或推送。后续阶段推进必须基于最新交接文档、当前仓库状态和用户最新指令，复用产品经理、UI 设计师、前端开发、后端开发、测试五个固定角色子代理。主代理只负责总体规划、范围控制、并行协调、审查汇总和交付判断，不因执行耗时、故障或返工压力直接接手业务实现。
