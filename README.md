...
当前稳定交付已经完成从 `000` 到 `023` 的基础阶段，并完成页面基础体验治理。`023 库存计价与仓储账实一致` 已交付公共移动加权平均、项目实际成本层、不可变价值流水、调拨、权属转换、盘点盘差、估值调整、成本权限和固定审批闭环。

后续 `024` 至 `036` 继续按“项目型制造的业财成本闭环优先”推进。下一阶段为 `024 采购深化与项目专采`，当前尚未启动；路线规划不代表功能已经实现。

2026-07-15 已完成正式验收环境全量重建：电气制造演示数据覆盖 `000` 至 `023`，Flyway V25，独立验证 114/114，MinIO 与数据库可用文件对象为 17/17。重建、备份、测试隔离和验收细节以当前交接及全量演示数据基线为准。
...
- 第一版以销售项目为一级成本和利润对象；项目合同与库存计价已在 020、023 完成，后续优先补齐项目专采、发票、正式项目成本、月结和正式财务。
