...
当前正式主线已经交付从 `000` 到 `032` 的业务阶段，并完成 025A、027A 两轮页面与现存问题治理。`024` 至 `027` 已形成项目采购、销售履约、订单驱动供给和项目生产/外协可信来源；`028 发票、费用与往来深化` 已交付业务结算来源；`029 项目成本核算` 已交付项目成本子账；`030 业务月结与期间快照` 已交付月末检查、原子关闭/重开、不可变多版本快照和八类固定经营报表基线；`031 会计科目、凭证与总账基础` 已正式交付单公司、单账簿、人民币的会计期间、科目、辅助核算、凭证、审批记账、总账、明细账、科目余额和试算平衡基础；`032 财务结账与资金税务基础` 已正式交付财务期间关闭与反结账、期末损益结转、银行对账和税务基础台账。当前 `033 项目利润与经营财务分析` 已在交付分支完成实现、集中审查、整改复审和唯一全量验证，待主代理最终合入交付。

`032 财务结账与资金税务基础` 已纯快进合入并推送 `main`，本地 `main`、`origin/main` 和实际远端均为 `c823861fd467b78119fb964a2f1d41314129186f`；正式 API `18080`、Web `5173` 均已核验可访问。033 的最终主分支哈希、远端同步状态和正式验收服务由主代理在最终合入后填写，本次产品文档同步不预写。后续 `034` 至 `036` 继续按“项目型制造的业财成本闭环优先”推进；下一业务阶段为 `034 平台交付治理完善`，尚未启动。

正式验收环境沿用受控电气制造演示数据，不因 024 至 032 前向迁移重建既有事实。正式库已由 Flyway V33 自然前迁至 V34；V29/V30/V31/V32/V33/V34 checksum 分别为 `774334682`、`2130342893`、`-2074547591`、`249406902`、`612501943`、`-629066235`，失败迁移 0，完整验证器 170/170。数据库 `AVAILABLE` 文件与 `qherp-private` 对象本次正式核验为 18/18；对象数量不是长期固定契约，后续合法业务文件可增长，但必须保持两侧动态一致且不少于 8。030 原 `OPEN/BLOCKED`、3 阻断/2 警告且无关闭/重开/快照事实保持不变；031 与 032 正式业务事实均为 0，未为验收补造结账、凭证、银行或税务事实。033 当前交付分支已完成 clean5 验证器 192/192 和受影响真实桌面路径最终核对；正式 V35 前迁、主分支服务和远端哈希仍以主代理最终合入交付记录为准。032 部署前备份目录为 `C:\Users\14567\.codex\backups\qherp\032-main-before-v34-20260721-043029`。运行入口、测试隔离和验收细节以当前交接为准。
...
- 033 的实现、集中审查、整改复审和唯一全量验证已经完成，待主代理最终合入交付；033 是 014 同一 `/reports`、`/api/admin/reports` 和 `reporting` 域内的模块化丰富，新增项目利润、合同回款、采购差异、库存资金、往来账龄、经营/会计对照和管理用固定经营财务摘要。V35 前向追加 8 个查看权限、7 个菜单项和 5 个 030 冻结报表代码；经营/会计对照和固定经营财务摘要不支持 `BUSINESS_SNAPSHOT`，财务摘要不是法定资产负债表、利润表或现金流量表。
