...
当前稳定交付已经完成从 `000` 到 `025` 的阶段，并完成页面基础体验治理。`024 采购深化与项目专采` 已交付请购、询价比价、价格协议、公共采购与项目专采、到货计划、入退库和有效采购供给；`025 销售深化与项目履约` 已交付销售报价、商业快照、受控订单变更、交付计划、商业信用、项目履约关闭和有效销售需求。

后续 `026` 至 `036` 继续按“项目型制造的业财成本闭环优先”推进。下一阶段为 `026 订单驱动缺料与供给建议`，当前尚未启动；路线规划不代表功能已经实现。

2026-07-15 已完成正式验收环境全量重建：电气制造演示数据覆盖 `000` 至 `023`，独立验证 114/114，MinIO 与数据库可用文件对象为 17/17；024、025 通过前向迁移交付且不重建既有演示事实，当前正式验收库为 Flyway V27。运行入口、测试隔离和验收细节以当前交接为准。
...
- 第一版以销售项目为一级成本和利润对象；项目合同、库存计价、项目专采和销售履约已在 020、023、024、025 完成，后续优先补齐订单驱动供给、项目生产、发票、正式项目成本、月结和正式财务。
