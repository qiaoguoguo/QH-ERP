...
当前已经正式交付从 `000` 到 `029` 的业务阶段，并完成 025A、027A 两轮页面与现存问题治理。`024` 至 `027` 已形成项目采购、销售履约、订单驱动供给和项目生产/外协可信来源；`028 发票、费用与往来深化` 已交付业务结算来源；`029 项目成本核算` 已交付以销售项目为一级成本对象的独立成本子账、来源快照、阶段归集、调整审批、差异和经营口径毛利。

后续 `030` 至 `036` 继续按“项目型制造的业财成本闭环优先”推进。下一业务阶段为 `030 业务月结与期间快照`，尚未启动目标讨论或冻结规格；路线规划不得表达为功能已经交付。

正式验收环境沿用受控电气制造演示数据，不因 024 至 029 前向迁移重建既有事实；029 正式部署后数据库版本为 Flyway V31，V29/V30/V31 checksum 分别为 `774334682`、`2130342893`、`-2074547591`，独立验证为 117/117。MinIO 与数据库可用文件对象保持两侧相等且不少于 8，当前交付基线为 18/18；对象数允许随合法附件、导入、导出或打印任务增长。运行入口、测试隔离和验收细节以当前交接为准。
