...
当前业务开发与验证已经完成从 `000` 到 `028` 的阶段，并完成 025A、027A 两轮页面与现存问题治理。`024` 至 `027` 已形成项目采购、销售履约、订单驱动供给和项目生产/外协可信来源；`028 发票、费用与往来深化` 已交付销售/采购发票、零容差三单匹配、外协采购侧结算、费用、预收预付、多目标核销、应收应付链接和非正式凭证草稿。主分支正式交付与运行状态以当前交接为准。
