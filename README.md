...
当前正式主线已经交付从 `000` 到 `034` 的业务与平台治理阶段，并完成 025A、027A 两轮页面与现存问题治理。`024` 至 `027` 已形成项目采购、销售履约、订单驱动供给和项目生产/外协可信来源；`028 发票、费用与往来深化` 已交付业务结算来源；`029 项目成本核算` 已交付项目成本子账；`030 业务月结与期间快照` 已交付月末检查、原子关闭/重开、不可变多版本快照和八类固定经营报表基线；`031 会计科目、凭证与总账基础` 已正式交付会计期间、凭证和总账基础；`032 财务结账与资金税务基础` 已正式交付财务关闭、期末损益、银行对账和税务基础；`033 项目利润与经营财务分析` 已在 014 经营报表基础上正式交付固定、可追溯的经营财务分析；`034 平台交付治理完善` 已交付数据修复、历史主数据导入、固定批量工具、固定打印、交付资料、隔离演示数据和操作手册治理。合并后的 `035 上线准备与最终生产交付` 已完成上线准备、候选冻结和 A01—A25 验证，等待主分支交付门禁 A26。

`034` 已于 2026-07-22 正式交付，业务交付节点为 `9d362b1cbbbddd0b6a68c837b58c0ca65e3d8059`。`035` 冻结候选为 `7d72a9e8fd8f46c316aa778e39d56c6af93e95b4`：V36、FullFacts 211/211、文件对象 41/41、后端 785 项、前端 1145 项、迁移/安全/性能/备份恢复、固定测试角色代码评审和真实桌面差异复验均已通过，阻断 0、严重 0、一般 0；在合入、远端同步和主分支服务核验前，当前稳定正式阶段仍按 034 记载。原 036 已合并进入 035 并废止独立编号。

默认正式环境沿用受控电气制造演示数据，不因 024 至 033 验收重建既有事实。默认正式库保持 Flyway V34、170/170 和 18/18，030 原 `OPEN/BLOCKED`、3 阻断/2 警告且无关闭/重开/快照事实保持不变，031/032 正式业务事实仍为 0。033 按阶段约束未写入或前迁默认正式库；V35、真实 2026-07 关闭、13 类快照、五类 `FROZEN` 与六类非空 LIVE 在隔离 clean5 主分支验收环境中完成 192/192。运行入口、测试隔离和验收细节以当前交接为准。
...
- 第一版以销售项目为一级成本和利润对象；项目合同、库存计价、项目专采、销售履约、订单驱动供给、项目生产/外协执行、业务发票费用往来及项目成本子账已在 020、023 至 029 完成，030 已补齐业务月结和期间快照，031 已正式补齐会计科目、凭证与总账基础，032 已正式补齐财务结账与资金税务基础，033 已在 014 经营报表中心内补齐固定项目利润与经营财务分析。
...
- 032 的 V34、集中审查、一次集中整改、差异复审、最终差异复核、唯一交付前全量验证、纯快进合入、正式 V34 自然前迁、完整验证器 170/170 和 12 个桌面入口核验已经完成；032 提供财务期间检查/关闭/反结账、期末损益结转、银行账户/流水/对账和税务基础汇总/缴纳台账，不提供完整财务报表、电子税务局申报、税控或红字发票平台。
- 033 的实现、集中审查、整改复审、唯一全量验证、纯快进合入、远端同步和主分支验收服务核验已经完成；033 是 014 同一 `/reports`、`/api/admin/reports` 和 `reporting` 域内的模块化丰富，新增项目利润、合同回款、采购差异、库存资金、往来账龄、经营/会计对照和管理用固定经营财务摘要。V35 前向追加 8 个查看权限、7 个菜单项和 5 个 030 冻结报表代码；经营/会计对照和固定经营财务摘要不支持 `BUSINESS_SNAPSHOT`，财务摘要不是法定资产负债表、利润表或现金流量表。
- 034 平台交付治理完善已经正式交付，范围限定为数据修复记录、五类历史主数据导入、固定批量工具、固定打印模板、交付资料、隔离演示数据和唯一操作手册治理。合并后的 035 只承担上线准备、候选冻结和最终生产交付验证，不新增业务功能；其候选与交付包现已冻结，只待主分支交付门禁。
...
- [033 项目利润与经营财务分析任务记录](docs/tasks/033-project-profit-operating-financial-analysis.md)
- [034 平台交付治理完善任务记录](docs/tasks/034-platform-delivery-governance.md)
- [035 上线准备与最终生产交付任务记录](docs/tasks/035-go-live-final-delivery.md)
