...
当前已经正式交付从 `000` 到 `030` 的业务阶段，并完成 025A、027A 两轮页面与现存问题治理。`024` 至 `027` 已形成项目采购、销售履约、订单驱动供给和项目生产/外协可信来源；`028 发票、费用与往来深化` 已交付业务结算来源；`029 项目成本核算` 已交付项目成本子账；`030 业务月结与期间快照` 已交付月末检查、原子关闭/重开、不可变多版本快照和八类固定经营报表基线。

后续 `031` 至 `036` 继续按“项目型制造的业财成本闭环优先”推进。下一业务阶段为 `031 会计科目、凭证与总账基础`，尚未启动目标讨论或冻结规格；路线规划不得表达为功能已经交付。

正式验收环境沿用受控电气制造演示数据，不因 024 至 030 前向迁移重建既有事实；030 正式部署后数据库版本为 Flyway V32，V29/V30/V31/V32 checksum 分别为 `774334682`、`2130342893`、`-2074547591`、`249406902`，独立验证为 127/127。MinIO 与数据库可用文件对象保持两侧相等且不少于 8，当前交付基线为 18/18；对象数允许随合法附件、导入、导出或打印任务增长。运行入口、测试隔离和验收细节以当前交接为准。
...
- 第一版以销售项目为一级成本和利润对象；项目合同、库存计价、项目专采、销售履约、订单驱动供给、项目生产/外协执行、业务发票费用往来及项目成本子账已在 020、023 至 029 完成，后续优先补齐业务月结、期间快照和正式财务。
