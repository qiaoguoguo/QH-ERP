...
正式验收环境沿用受控电气制造演示数据，不因 024 至 026 前向迁移重建既有事实；当前正式库为 Flyway V28，独立验证 114/114。026 验收导出新增 1 个合法规划 Excel 后，MinIO 与数据库可用文件对象当前为 18/18；对象数允许随合法附件、导入、导出或打印任务增长，但必须始终保持两侧相等且不少于 8。运行入口、测试隔离和验收细节以当前交接为准。
