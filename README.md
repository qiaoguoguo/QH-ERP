...
当前已经正式交付从 `000` 到 `030` 的业务阶段，并完成 025A、027A 两轮页面与现存问题治理。`024` 至 `027` 已形成项目采购、销售履约、订单驱动供给和项目生产/外协可信来源；`028 发票、费用与往来深化` 已交付业务结算来源；`029 项目成本核算` 已交付项目成本子账；`030 业务月结与期间快照` 已交付月末检查、原子关闭/重开、不可变多版本快照和八类固定经营报表基线。`031 会计科目、凭证与总账基础` 已在功能分支 `codex/031-accounting-ledger-foundation` 完成实现、全量验证和最终证据刷新，待主代理合入、推送并正式部署 V33。

后续 `032` 至 `036` 继续按“项目型制造的业财成本闭环优先”推进。下一业务阶段为 `032 财务结账与资金税务基础`；路线规划不得提前表达为功能已经交付。

正式验收环境沿用受控电气制造演示数据，不因 024 至 031 前向迁移重建既有事实。当前正式环境仍为 Flyway V32，待主代理合入部署后自然前迁 V33；V29/V30/V31/V32 checksum 分别为 `774334682`、`2130342893`、`-2074547591`、`249406902`，V33 checksum 为 `612501943`。031 功能分支隔离验证为 140/140（138 SQL + 2 环境），GL 门禁为 `activeRules=7`、`activeLines=17`、`activeAuxMaps=9` 且违反数为 0；正式含 MinIO 部署验收门禁为 142/142、对象 18/18，不得在部署前写成已发生事实。运行入口、测试隔离和验收细节以当前交接为准。
...
- 第一版以销售项目为一级成本和利润对象；项目合同、库存计价、项目专采、销售履约、订单驱动供给、项目生产/外协执行、业务发票费用往来及项目成本子账已在 020、023 至 029 完成，030 已补齐业务月结和期间快照，031 已在功能分支补齐正式会计科目、凭证与总账基础，后续进入 032 财务结账与资金税务基础。
...
- 031 的功能分支实现、V33、集中审查整改、差异复审、唯一全量窗口、整改复验、140/140 隔离验证和真实桌面核验已经完成；待主代理合入、推送和正式部署。031 提供单公司单账簿人民币的会计科目、凭证、审批记账、总账/明细账/余额/试算基础，不等同 032 财务结账与资金税务基础。
