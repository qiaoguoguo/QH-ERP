...
public class PlatformAttachmentService {
	...
	private static final List<String> ALLOWED_EXTENSIONS = List.of("txt", "pdf", "png", "jpg", "jpeg", "xlsx");
	...
	public AttachmentRecord upload(AttachmentUpload upload, CurrentUser operator, HttpServletRequest servletRequest) {
		...
		String objectKey = "attachments/" + upload.objectType() + "/" + upload.objectId() + "/" + UUID.randomUUID();
		...
		try {
			Long fileId = this.jdbcTemplate.queryForObject("""
					...
					upload.contentType(), upload.content().length, sha256, storedObject.eTag(), operator.id(),
	...
	public record AttachmentRecord(Long id, String objectType, Long objectId, String originalFilename,
			String contentType, Long sizeBytes, String sha256, String description, String status, String createdBy,
