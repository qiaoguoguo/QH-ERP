...
async function loadRecords() {
  loading.value = true
  try {
    const page = await financeSettlementApi.advanceReceipts.list({
  ...
  <MasterDataTableView title="预收款" description="展示已过账收款形成的客户未核销余额。">
    ...
    <template #filters>
      <el-form class="query-form" inline>
        ...
        <el-form-item label="客户"><el-input v-model="filters.customerId" clearable placeholder="选择客户" /></el-form-item>
        ...
        <el-form-item label="收款日期"><el-date-picker v-model="filters.businessDateFrom" value-on-clear="" type="date" format="YYYY-MM-DD" value-format="YYYY-MM-DD" placeholder="起始日期" /></el-form-item>
    ...
    <template #alerts><el-alert v-if="error" type="error" :title="error" :closable="false" /><el-alert v-if="loading" type="info" title="预收款加载中" :closable="false" /></template>
    <el-empty v-if="!loading && records.length === 0" description="暂无预收款" />
    <div class="table-scroll">
      <el-table :data="records" :empty-text="loading ? '加载中' : '暂无预收款'">
        <el-table-column prop="advanceNo" label="预收单号" min-width="150" />
        <el-table-column prop="fundNo" label="收款单号" min-width="150" />
        <el-table-column prop="partnerName" label="客户" min-width="150" />
        ...
        <el-table-column prop="businessDate" label="收款日期" min-width="110" />
        <el-table-column label="收款金额" min-width="120" align="right"><template #default="{ row }">{{ formatFinanceAmount(row.amount) }}</template></el-table-column>
        ...
        <el-table-column label="操作" fixed="right" min-width="110"><template #default="{ row }"><el-button text @click="router.push({ name: 'finance-advance-receipt-detail', params: { id: row.id } })">详情</el-button></template></el-table-column>
      </el-table>
    </div>
