...
const canEdit = computed(() => record.value?.allowedActions?.includes('UPDATE') && authStore.hasPermission(financePermissions.prepaymentUpdate))
const canPost = computed(() => record.value?.allowedActions?.includes('POST') && authStore.hasPermission(financePermissions.prepaymentPost))
const canCancel = computed(() => record.value?.allowedActions?.includes('CANCEL') && authStore.hasPermission(financePermissions.prepaymentCancel))
...
function prepaymentPartyName(fund: AdvanceFundRecord) {
  return firstDisplayText(fund.supplierName, fund.partnerName) || '供应商待补全'
}
...
function fundSourceSummary(fund: AdvanceFundRecord) {
  ...
  return `${fund.advanceNo} 由付款单 ${fund.fundNo || '来源单据待补全'} 形成，不新增现金发生额。`
}
...
async function loadRecord() {
  ...
  error.value = ''
  try {
    record.value = await financeSettlementApi.prepayments.get(route.params.id as string)
  } catch (caught) {
...
async function runFundAction(action: 'post' | 'cancel') {
  ...
  if (!(await confirmAction(`${label}预付款“${record.value.advanceNo}”？`))) {
    return
  }
  ...
  try {
    const payload = {
      ...
      idempotencyKey: `${action}-prepayment-${record.value.id}-${Date.now()}`,
    }
    if (action === 'post') {
      await financeSettlementApi.prepayments.post(record.value.id, payload)
    } else {
      await financeSettlementApi.prepayments.cancel(record.value.id, payload)
    }
    await loadRecord()
  } catch (caught) {
...
<template>
  <MasterDataTableView title="预付款详情" description="查看供应商真实付款资金、可用余额、已核销目标和凭证草稿摘要。">
    <template #actions>
      <el-button @click="router.push({ name: 'finance-prepayments' })">返回列表</el-button>
      <el-button v-if="canEdit" @click="router.push({ name: 'finance-prepayment-edit', params: { id: route.params.id } })">编辑草稿</el-button>
      <el-button v-if="canPost" data-test="post-prepayment" type="success" :loading="actionLoading" :disabled="actionLoading" @click="runFundAction('post')">过账</el-button>
      <el-button v-if="canCancel" data-test="cancel-prepayment" type="danger" :loading="actionLoading" :disabled="actionLoading" @click="runFundAction('cancel')">取消草稿</el-button>
    </template>
    <template #alerts>
      <el-alert v-if="error" type="error" :title="error" :closable="false" />
      <el-alert v-if="actionError" type="error" :title="actionError" :closable="false" />
      <el-alert v-if="loading" type="info" title="预付款详情加载中" :closable="false" />
    ...
    <div v-if="record" class="finance-summary-strip">
      ...
      <div><span>供应商</span><strong>{{ prepaymentPartyName(record) }}</strong></div>
    ...
    <div v-if="record" class="finance-section-grid">
      ...
      <section class="finance-section">
        ...
        <el-button data-test="start-prepayment-allocation" type="primary" :disabled="!canAllocate" @click="openSettlementWorkbench">发起核销</el-button>
      </section>
      <section class="finance-section">
