...
const canCreate = computed(() => authStore.hasPermission('master:supplier:create'))
const canUpdate = computed(() => authStore.hasPermission('master:supplier:update'))
const canGenerateCode = computed(() => canCreate.value && authStore.hasPermission('master:coding-rule:generate'))
const canSettlementView = computed(() => authStore.hasPermission('master:supplier-settlement:view'))
const canSettlementUpdate = computed(() => authStore.hasPermission('master:supplier-settlement:update'))
const canSettlementSensitiveUpdate = computed(() => authStore.hasPermission('master:supplier-settlement:sensitive-update'))
...
async function openSettlement(record: PartnerRecord) {
  ...
  try {
    const detail = await masterDataApi.suppliers.getSettlementTax(record.id)
...
async function saveSettlement() {
  ...
  try {
    settlementRecord.value = await masterDataApi.suppliers.updateSettlementTax(settlementTarget.value.id, payload)
...
async function saveRecord() {
  ...
  try {
    if (editingRecord.value) {
      await masterDataApi.suppliers.update(editingRecord.value.id, payload)
    } else {
      await masterDataApi.suppliers.create(payload)
    }
...
async function generateCode() {
  ...
  try {
    const result = await masterDataApi.codingRules.generate({ objectType: 'SUPPLIER' })
...
async function changeStatus(record: PartnerRecord) {
  ...
  if (!(await confirmAction(`确认${nextAction}供应商“${record.name}”？`))) {
    return
  }
  actionError.value = ''
  ...
  try {
    if (record.status === 'DISABLED') {
      await masterDataApi.suppliers.enable(record.id)
    } else {
      await masterDataApi.suppliers.disable(record.id)
    }
    await loadRecords()
  } catch (caught) {
...
<template>
  <MasterDataTableView title="供应商" description="维护采购和委外业务使用的供应商基础资料。">
    <template #actions>
      <el-button v-if="canCreate" data-test="create-record" type="primary" @click="openCreate">新增供应商</el-button>
      <a
      ...
      <BatchStatusToolPanel
        ...
        tool-code="SUPPLIER_STATUS_CHANGE_V1"
        title="供应商批量状态"
        button-test-id="supplier-batch-status-entry"
    ...
    <template #alerts>
      ...
      <el-alert v-if="loading" class="state-alert" type="info" title="供应商数据加载中" :closable="false" />
    </template>
        ...
        <el-table-column label="操作" fixed="right" width="184">
          <template #default="{ row }">
            ...
            <el-button
              ...
              size="small"
              text
              data-test="edit-supplier-settlement-tax"
    ...
    <el-dialog v-model="formVisible" :title="editingRecord ? '编辑供应商' : '新增供应商'" width="560px">
        ...
        <el-form-item label="供应商编码">
          <div class="field-with-action">
            ...
            <el-button
              ...
              data-test="generate-supplier-code"
    ...
    <el-dialog v-model="settlementVisible" title="供应商结算税务资料" width="min(760px, 96vw)">
