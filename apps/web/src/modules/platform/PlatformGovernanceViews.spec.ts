...
describe('034 平台治理页面', () => {
  ...
  it('数据修复详情展示前后摘要、证据、审批、验证和不可变时间线，高风险执行走确认和版本幂等', async () => {
    ...
    expect(wrapper.text()).toContain('预检查')
    expect(wrapper.text()).toContain('通过')
    expect(wrapper.text()).toContain('草稿 至 待审批')
    expect(wrapper.text()).toContain('已通过')
    expect(wrapper.text()).not.toContain('PRECHECK')
    expect(wrapper.text()).not.toContain('PASSED')
    expect(wrapper.text()).not.toContain('DRAFT 至 PENDING_APPROVAL')
  ...
  it('交付资料页为只读目录，展示模板、适配器、批量工具、手册和演示数据版本', async () => {
    ...
    expect(wrapper.text()).not.toContain('NOT_VERIFIED')
    expect(wrapper.text()).toContain('状态变更')
    expect(wrapper.text()).not.toContain('ACTIVE')
    const actionColumns = wrapper.findAllComponents({ name: 'ElTableColumn' })
      .map((column) => column.props() as Record<string, unknown>)
      .filter((column) => column.label === '动作')
    expect(actionColumns).toHaveLength(1)
    expect(actionColumns[0].prop).toBeUndefined()
  ...
  it('批量状态工具面板用真实候选搜索和分页追加预检，按 availableActions 确认执行', async () => {
    ...
    expect(wrapper.text()).not.toContain('PRECHECKED')
