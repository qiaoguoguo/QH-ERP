...
async function confirmTask() {
  ...
  try {
    currentTask.value = await documentPlatformApi.imports.confirm(currentTask.value.id, {
      ...
      idempotencyKey: createIdempotencyKey('procurement-document-confirm'),
    })
  } catch (caught) {
    actionError.value = platformErrorMessage(caught)
  } finally {
...
<template>
  <section v-if="currentTask" class="procurement-document-task-panel">
    ...
    <div class="task-summary">
      <div>
        ...
        <span v-if="currentTask.expiresAt">结果过期：{{ formatPlatformDateTime(currentTask.expiresAt) }}</span>
      ...
      <div class="task-actions">
        <el-button
          ...
          data-test="procurement-task-confirm"
          size="small"
          type="primary"
        ...
        <el-button
          v-if="hasAction('ERRORS')"
          data-test="procurement-task-errors"
          size="small"
          @click="openErrors"
        >
          错误明细
        ...
        <el-button
          ...
          data-test="procurement-task-download"
          size="small"
          :loading="actionLoading"
          @click="downloadTask"
        >
        ...
        <el-button
          ...
          data-test="procurement-task-cancel"
          size="small"
          type="danger"
          :loading="actionLoading"
          @click="cancelTask"
...
<style scoped>
.procurement-document-task-panel {
  display: grid;
  gap: 10px;
}
