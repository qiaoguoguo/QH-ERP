...
describe('报表页面规范与中文状态治理', () => {
  ...
  it('共享状态和来源 helper 不把未知枚举作为用户主文案', () => {
    ...
    expect(reportTraceStatusText({ sourceType: 'PROJECT_COST_SOURCE_LINE', status: 'ACTUAL' })).toBe('实际')
    expect(reportTraceStatusText({ sourceType: 'PROJECT_COST_SOURCE_LINE', status: 'PROVISIONAL' })).toBe('暂估')
    expect(reportTraceStatusText({ sourceType: 'PROJECT_COST_SOURCE_LINE', status: 'UNPRICED' })).toBe('未定价')
    expect(reportTraceStatusText({ sourceType: 'PROJECT_COST_SOURCE_LINE', status: 'ADJUSTED' })).toBe('已调整')
    expect(reportTraceStatusText({ sourceType: 'PROJECT_COST_SOURCE_LINE', status: 'RESTRICTED' })).toBe('来源受限')
    expect(reportTraceStatusText({ sourceType: 'PROJECT_COST_SOURCE_LINE', status: 'EXCLUDED' })).toBe('已排除')
    expect(reportTraceStatusText({ sourceType: 'PROJECT_COST_SOURCE_LINE', status: 'ACTUAL', statusName: '服务端实际' })).toBe('服务端实际')
    expect(reportTraceStatusText({ sourceType: 'PROJECT_COST_SOURCE_LINE', status: 'ACTUAL', statusName: 'ACTUAL' })).toBe('实际')
