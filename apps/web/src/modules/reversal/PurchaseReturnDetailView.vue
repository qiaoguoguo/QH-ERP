...
function editPurchaseReturn() {
  ...
  void router.push({ name: 'procurement-return-edit', params: { id: String(record.value.id) } })
}
...
async function postPurchaseReturn() {
  if (!record.value || actionLoading.value || !window.confirm(`确认过账采购退货“${record.value.returnNo}”？`)) {
    return
  }
  actionError.value = ''
...
async function cancelPurchaseReturn() {
  if (!record.value || actionLoading.value || !window.confirm(`确认取消采购退货“${record.value.returnNo}”？`)) {
    return
  }
  actionError.value = ''
  actionLoading.value = true
