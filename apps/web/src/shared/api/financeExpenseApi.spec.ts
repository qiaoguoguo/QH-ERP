...
import { createFinanceExpenseApi, type ExpensePayload, type ExpenseSourceType } from './financeExpenseApi'
...
describe('028 费用分域 API', () => {
  it('费用来源类型不暴露后端不支持的外协订单', () => {
    const typeContract: {
      outsourcingOrderDoesNotExposeAsExpenseSource: AssertTrue<'OUTSOURCING_ORDER' extends ExpenseSourceType ? false : true>
    } = {
      outsourcingOrderDoesNotExposeAsExpenseSource: true,
    }
  ...
  it('查询费用、分类和来源候选时使用独立资源端点且来源候选透传页面筛选', async () => {
    const fetcher = vi.fn()
