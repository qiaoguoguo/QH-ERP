...
describe('页面治理路由清单门禁', () => {
  it('生成 236 个生产唯一路由并解释排除 spec 后的口径', () => {
    const inventory = buildRouteInventory()

    expect(inventory.routes, summarizeRouteInventory(inventory)).toHaveLength(236)
  ...
  it('解释 system 兼容别名但不把别名计入生产主路由', () => {
    ...
    expect(inventory.topLevelCounts['/system']).toBe(4)
  })
  ...
  it('动态 import 指向存在的 Vue 页面文件', () => {
    ...
    expect(directRouteComponentImports, summarizeRouteInventory(inventory)).toHaveLength(207)
    expect(inventory.dynamicImports, summarizeRouteInventory(inventory)).toHaveLength(223)
