...
- [031 会计科目、凭证与总账基础任务记录（功能分支已实现并验证，待主代理合入部署）](tasks/031-accounting-voucher-general-ledger-foundation.md)

021 起采用单一权威阶段说明：对应任务记录同时承载产品契约、接口、数据迁移、权限、工作包、集中审查和验收结果，不再重复维护含义相同的独立规格、实施计划或测试文档。031 完成合入部署前仍以功能分支证据为准；下一阶段指向 032 财务结账与资金税务基础，不得提前宣称已交付。
