...
> 历史兼容说明：本文后部保留 013 阶段初始接口契约。015 已通过 V11 增加 `adjusted_amount` 与往来冲减；028 通过 V30 增加业务发票、费用、预收预付、多目标核销和凭证草稿。现行余额恒等式为“应收总额 = 已收 + 累计调整 + 未收”“应付总额 = 已付 + 累计调整 + 未付”；涉及现行接口、调整额、核销和当前余额约束时，以本节 028 契约、`docs/api/return-refund-reversal-api.md`、当前代码和当前验收标准为准，不得继续使用本文后部 013 单目标初始规则作为现行限制。

## 028 当前资源与契约

### 稳定资源组

- `/api/admin/finance/sales-invoices`：销售出库候选、列表、详情、创建/更新草稿、确认、取消、来源追溯和应收链接。
- `/api/admin/finance/purchase-invoices`：标准采购/外协候选、列表、详情、创建/更新草稿、三单匹配、确认、取消、来源追溯和应付链接。
- `/api/admin/finance/expenses`：费用分类与来源候选、列表、详情、创建/更新草稿、确认和取消。
- `/api/admin/finance/advance-receipts`、`/api/admin/finance/prepayments`：列表、详情、创建/更新资金草稿、过账、取消、可用余额和核销记录。
- `/api/admin/finance/settlement-workbench`：客户/供应商方向汇总、候选资金与目标、核销草稿、过账、取消和追溯。
- `/api/admin/finance/voucher-drafts`：列表、详情、从来源幂等生成、标记待正式制证、取消和来源追溯。
- 既有 `/receivables`、`/receipts`、`/payables`、`/payments` 和往来冲减接口保持兼容；历史嵌套单目标入口继续调用同一余额服务，028 通用入口允许一笔资金原子分配多个目标并保留未核销预收/预付余额。

### 现行业务规则

- 销售发票只允许已过账销售出库来源；确认时优先绑定既有应收，缺失时按来源唯一生成，不能重复增加余额。
- 标准采购发票只允许采购订单与已过账采购入库来源，按数量、未税/含税单价及金额零容差匹配；任何结构化差异都返回稳定匹配结果并禁止确认。
- 外协采购发票只允许已过账外协收货来源，继承供应商、项目/公共归属、数量和金额；不适用标准采购订单三单匹配，创建、确认和结算不得回写库存数量价值。
- 费用允许启用供应商及可选采购/外协来源，必须明确项目或公共归属；确认形成或绑定应付，但不写 029 正式项目成本。
- 真实收款/付款的金额等于累计核销金额与未核销余额之和。预收/预付核销只能在同主体、人民币、同项目/公共归属内分配，不能再次增加现金；超额、负余额、并发版本或任一目标失败必须整体回滚。
- 凭证草稿只包含业务分类、借贷方向、金额、来源、往来方和项目/公共归属，不产生正式科目、会计期间、凭证号、记账或总账。
- V10/V11 历史应收、应付、收付款和调整继续作为余额权威；V30 不回填历史发票、项目归属或凭证草稿。

### 请求、响应、权限与错误

- 所有业务金额与数量继续使用十进制字符串；前端不得用浮点数计算可信金额。所有写请求携带 `version` 与 `idempotencyKey`，陈旧版本、同键异载荷或重复业务来源必须稳定拒绝或返回同一幂等结果。
- 详情响应至少返回业务状态、派生结算/匹配状态、`version`、`allowedActions`、来源摘要、十进制金额、项目/公共归属和权限受限原因；无权字段不得用 0、空串或内部 ID 伪装。
- 权限组为 `finance:sales-invoice:*`、`finance:purchase-invoice:*`、`finance:expense:*`、`finance:advance-receipt:*`、`finance:prepayment:*`、`finance:settlement-allocation:*`、`finance:voucher-draft:*`，以及独立的 `finance:settlement-sensitive:view/update`。列表主操作、路由和接口必须一致消费权限，后端鉴权是最终边界。
- 稳定错误至少区分不存在、状态不允许、来源不一致、匹配差异、累计超额、余额不足、跨主体/项目、并发冲突、幂等冲突、期间锁定、权限不足和敏感字段受限；失败不得产生部分写入。

### 028 明确排除

- 不提供员工报销、红字发票、税务申报、多币种、银行对账、正式项目成本、月结、正式会计科目/凭证、总账或关账。
- 业务发票不等同正式收入确认；凭证草稿不等同正式凭证；费用/外协结算归属不等同 029 项目成本。

## 013 历史初始契约（保留）

以下内容记录 013 阶段当时的单目标应收、收款、应付和付款接口，不随 028 追改；与上文 028 当前契约冲突时，以上文和当前代码为准。
