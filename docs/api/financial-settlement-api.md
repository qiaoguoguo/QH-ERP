...
> 历史兼容说明：本文保留 013 阶段初始接口契约。015 已通过 V11 增加 `adjusted_amount` 与往来冲减，当前余额恒等式为“应收总额 = 已收 + 累计调整 + 未收”“应付总额 = 已付 + 累计调整 + 未付”；涉及调整额、冲减和当前余额约束时，以 `docs/api/return-refund-reversal-api.md`、当前代码和当前验收标准为准，不得继续使用本文后部 013 初始恒等式作为现行规则。
