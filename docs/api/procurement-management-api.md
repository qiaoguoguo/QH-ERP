...
定义采购管理基础阶段的接口范围、字段、权限、错误码和联调验收规则，作为前后端实现与测试依据。本阶段只建立采购订单、采购入库、库存增加和来源追溯，不提供应付、发票、付款、税务、审批、询价、比价、供应商评分、MRP、质检入库、采购退货、暂估入库、多币种、多组织、多公司、多账套、多租户、正式财务凭证或报表分析能力。
