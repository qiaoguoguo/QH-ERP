...
- 财务往来基础模块实现提交：`90c3785 完成财务往来基础模块`。
- 当前阶段：`013 财务往来基础模块` 已完成开发、审查、自动化测试、本地浏览器验收、视觉分析和固定角色最终复审。
- 当前工作区状态：财务往来基础模块已经提交到当前分支，提交后工作区干净；尚未合入主分支，尚未推送远端分支。
- 当前本地验收服务：
...
接手后第一优先级不是立即扩展新功能，而是确认当前财务往来基础模块已提交后的交付状态，并根据用户指令决定是否合入主分支、推送远端分支和准备主分支阶段验收。
...
4. 运行或复核当前质量门，至少包括：
   ...
   - `git status --short`。
   - `git diff --check`。
5. 确认当前分支至少包含提交 `90c3785 完成财务往来基础模块`。
6. 不要在未确认当前交付状态前启动下一业务模块。
7. 若用户要求阶段交付或合入，先处理主分支合入或远端推送，再在主分支本地部署并提供浏览器验收地址。
...
财务往来基础模块已经通过提交 `90c3785 完成财务往来基础模块` 写入当前分支。提交统计为 91 个文件变更，新增 13350 行，删除 4 行。提交后 `git status --short --branch` 只显示当前分支，工作区和暂存区均无未提交内容。
...
注意：视觉目录中包含 `task11-result.json`，它是 Task 11 视觉执行结果摘要，当前作为视觉证据辅助文件保留。
...
1. 复核当前分支和提交，确认至少包含 `90c3785 完成财务往来基础模块`。
2. 复核 `git status --short --branch`，确认工作区干净。
3. 复跑最低质量门：
   - `git diff --check`
   - 后端健康检查。
   - 前端访问检查。
4. 如用户要求远端或新机器接手，推送当前分支或合入主分支后推送。
5. 合入或推送前，按项目规则确认目标分支稳定。
6. 合入主分支后必须在主分支本地启动服务并提供浏览器验收地址。
7. 不得把未验证分支直接推给用户验收。

### 如果目标是开启下一阶段开发

必须先完成当前财务往来阶段的合入、推送或主分支验收决策，或至少得到用户明确授权可以在当前分支基础上继续新阶段。
...
财务往来基础模块已经完成到本地浏览器验收候选状态，并已通过提交 `90c3785 完成财务往来基础模块` 写入当前分支。当前分支包含完整代码、文档、测试、浏览器验收和视觉证据；尚未合入主分支，尚未推送远端分支。

新主代理接手后的首要任务是保护当前已验证成果：先复核当前分支、最新提交、干净工作区和质量门，再按用户指令完成合入、推送或主分支阶段验收。只有在当前阶段交付边界明确之后，才应启动下一阶段。
