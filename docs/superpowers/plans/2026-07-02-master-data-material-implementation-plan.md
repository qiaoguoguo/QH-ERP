# 基础资料与物料管理 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 建立计量单位、仓库、供应商、客户、物料分类、物料档案的主数据维护能力，为后续 BOM、库存和生产工单提供稳定数据底座。

**Architecture:** 后端沿用当前 Spring Boot + JdbcTemplate + Flyway + 集中权限过滤器模式，新增 `master` 领域的迁移、服务、控制器、权限种子和集成测试。前端沿用 Vue 3 + Element Plus + Pinia 认证状态 + 路由守卫模式，新增基础资料与物料管理页面，页面状态保持局部化，通用表格/表单模式从账号权限页面提炼。

**Tech Stack:** Java 21、Spring Boot、JdbcTemplate、Flyway、PostgreSQL、JUnit/Testcontainers、Vue 3、TypeScript、Element Plus、Vitest、Vite。
...
### Task 1: 任务文档、接口契约和测试计划

**Files:**
- Create: `docs/tasks/005-master-data-material-foundation.md`
- Create: `docs/api/master-data-material-api.md`
- Create: `docs/testing/master-data-material-test-plan.md`
- Modify: `docs/product/product-decisions.md`

- [ ] **Step 1: 写任务文档**

Create `docs/tasks/005-master-data-material-foundation.md` with this structure:
...
Create `docs/api/master-data-material-api.md` with endpoint tables for:
...
Modify `docs/product/product-decisions.md`, add:
...
**Files:**
- Create: `apps/api/src/main/resources/db/migration/V3__master_data_material_schema.sql`
- Create: `apps/api/src/main/java/com/qherp/api/system/master/MasterDataStatus.java`
- Create: `apps/api/src/main/java/com/qherp/api/system/master/MaterialType.java`
- Create: `apps/api/src/main/java/com/qherp/api/system/master/MaterialSourceType.java`
...
- Modify: `apps/api/src/test/java/com/qherp/api/system/init/AccountPermissionInitializerTests.java`

- [ ] **Step 1: 写迁移测试期望**

Modify `apps/api/src/test/java/com/qherp/api/system/init/AccountPermissionInitializerTests.java`, add assertions to existing initializer test:
...
Expected: FAIL because new permissions are not initialized.
...
Modify `apps/api/src/main/java/com/qherp/api/common/ApiErrorCode.java`, add enum values:
...
Modify `apps/api/src/main/java/com/qherp/api/system/init/AccountPermissionInitializer.java`, append permission seeds after `system:audit:view`:
...
**Files:**
- Create: `apps/api/src/main/java/com/qherp/api/system/master/MasterDataAdminService.java`
- Create: `apps/api/src/main/java/com/qherp/api/system/master/UnitAdminController.java`
- Create: `apps/api/src/main/java/com/qherp/api/system/master/WarehouseAdminController.java`
- Create: `apps/api/src/main/java/com/qherp/api/system/master/SupplierAdminController.java`
- Create: `apps/api/src/main/java/com/qherp/api/system/master/CustomerAdminController.java`
- Create: `apps/api/src/test/java/com/qherp/api/system/master/MasterDataAdminControllerTests.java`
- Modify: `apps/api/src/main/java/com/qherp/api/security/PermissionAuthorizationManager.java`

- [ ] **Step 1: 写失败集成测试**

Create `apps/api/src/test/java/com/qherp/api/system/master/MasterDataAdminControllerTests.java` with tests:
...
class MasterDataAdminControllerTests extends PostgresIntegrationTest {
	...
	void duplicateCodeReturnsConflict() {
		AuthenticatedSession admin = login("admin", "Qherp@2026!");
		Map<String, Object> request = Map.of("code", "DUP-UNIT", "name", "重复单位", "precisionScale", 0, "status", "ENABLED");
	...
	void unauthorizedUserCannotCreateMasterData() {
		...
		ResponseEntity<String> forbidden = exchange(HttpMethod.POST, "/api/admin/master/units",
				Map.of("code", "NO-AUTH", "name", "无权限", "precisionScale", 0, "status", "ENABLED"), readonly);
...
Create `apps/api/src/main/java/com/qherp/api/system/master/MasterDataAdminService.java` with:
...
public class MasterDataAdminService {
	...
	public PageResponse<RecordResponse> list(Resource resource, String keyword, String status, int page, int pageSize) {
		...
		List<RecordResponse> items = this.jdbcTemplate.query(resource.selectSql(queryParts.where), this::mapRecord, args.toArray());
		return PageResponse.of(items, page, Math.min(Math.max(pageSize, 1), 100), total);
	}

	@Transactional(readOnly = true)
	public RecordResponse get(Resource resource, Long id) {
		return this.jdbcTemplate.query(resource.selectOneSql(), this::mapRecord, id).stream().findFirst()
			.orElseThrow(() -> new BusinessException(ApiErrorCode.MASTER_DATA_NOT_FOUND));
	}

	@Transactional
	public RecordResponse create(Resource resource, RecordRequest request, CurrentUser operator, HttpServletRequest servletRequest) {
		OffsetDateTime now = OffsetDateTime.now();
		try {
			Long id = this.jdbcTemplate.queryForObject(resource.insertSql(), Long.class, request.code(), request.name(),
					request.extra1(), request.extra2(), request.extra3(), statusOrEnabled(request.status()).name(),
					request.remark(), operator.username(), now, operator.username(), now);
	...
	@Transactional
	public RecordResponse update(Resource resource, Long id, RecordRequest request, CurrentUser operator,
			HttpServletRequest servletRequest) {
		validateExists(resource, id);
		try {
			int updated = this.jdbcTemplate.update(resource.updateSql(), request.name(), request.extra1(), request.extra2(),
					request.extra3(), statusOrEnabled(request.status()).name(), request.remark(), operator.username(),
	...
	@Transactional
	public RecordResponse enable(Resource resource, Long id, CurrentUser operator, HttpServletRequest servletRequest) {
	...
	private RecordResponse changeStatus(Resource resource, Long id, MasterDataStatus status, CurrentUser operator,
			HttpServletRequest servletRequest) {
		validateExists(resource, id);
	...
	private RecordResponse mapRecord(ResultSet rs, int rowNum) throws SQLException {
		return new RecordResponse(rs.getLong("id"), rs.getString("code"), rs.getString("name"), rs.getString("extra1"),
				rs.getString("extra2"), rs.getString("extra3"), rs.getString("status"), rs.getString("remark"),
	...
	public enum Resource {
		UNIT("mst_unit", "precision_scale::text", "sort_order::text", "null::text", "UNIT", "UNIT_CREATE", "UNIT_UPDATE", "UNIT_ENABLE", "UNIT_DISABLE"),
		WAREHOUSE("mst_warehouse", "warehouse_type", "manager_name", "address", "WAREHOUSE", "WAREHOUSE_CREATE", "WAREHOUSE_UPDATE", "WAREHOUSE_ENABLE", "WAREHOUSE_DISABLE"),
		SUPPLIER("mst_supplier", "contact_name", "contact_phone", "null::text", "SUPPLIER", "SUPPLIER_CREATE", "SUPPLIER_UPDATE", "SUPPLIER_ENABLE", "SUPPLIER_DISABLE"),
		CUSTOMER("mst_customer", "contact_name", "contact_phone", "null::text", "CUSTOMER", "CUSTOMER_CREATE", "CUSTOMER_UPDATE", "CUSTOMER_ENABLE", "CUSTOMER_DISABLE");

		private final String table;
		private final String extra1Select;
		private final String extra2Select;
		private final String extra3Select;
		...
		Resource(String table, String extra1Select, String extra2Select, String extra3Select, String auditTarget,
				String auditCreate, String auditUpdate, String auditEnable, String auditDisable) {
			this.table = table;
			this.extra1Select = extra1Select;
			this.extra2Select = extra2Select;
			this.extra3Select = extra3Select;
		...
		private String selectSql(String where) {
			return "select id, code, name, " + extra1Select + " as extra1, " + extra2Select + " as extra2, " + extra3Select
					+ " as extra3, status, remark, created_at, updated_at from " + table + " " + where + " order by id desc limit ? offset ?";
		}

		private String selectOneSql() {
			return "select id, code, name, " + extra1Select + " as extra1, " + extra2Select + " as extra2, " + extra3Select
					+ " as extra3, status, remark, created_at, updated_at from " + table + " where id = ?";
		}
		...
		private String updateSql() {
			return switch (this) {
				case UNIT -> "update mst_unit set name = ?, precision_scale = cast(? as integer), sort_order = cast(? as integer), remark = ?, status = ?, updated_by = ?, updated_at = ?, version = version + 1 where id = ?";
				case WAREHOUSE -> "update mst_warehouse set name = ?, warehouse_type = ?, manager_name = ?, address = ?, status = ?, remark = ?, updated_by = ?, updated_at = ?, version = version + 1 where id = ?";
				case SUPPLIER -> "update mst_supplier set name = ?, contact_name = ?, contact_phone = ?, status = ?, remark = ?, updated_by = ?, updated_at = ?, version = version + 1 where id = ?";
				case CUSTOMER -> "update mst_customer set name = ?, contact_name = ?, contact_phone = ?, status = ?, remark = ?, updated_by = ?, updated_at = ?, version = version + 1 where id = ?";
			};
		}
	}

	public record RecordRequest(@NotBlank String code, @NotBlank String name, String extra1, String extra2,
			String extra3, String status, String remark) {
	}

	public record RecordResponse(Long id, String code, String name, String extra1, String extra2, String extra3,
			String status, String remark, OffsetDateTime createdAt, OffsetDateTime updatedAt) {
	}
}
```

If Java compilation fails around enum private fields access, make the `Resource` fields package-private by removing `private final`.

- [ ] **Step 4: 创建四个控制器**

Create `UnitAdminController.java`:
...
public class UnitAdminController {
	...
	@GetMapping
	public ApiResponse<PageResponse<MasterDataAdminService.RecordResponse>> list(@RequestParam(required = false) String keyword,
	...
	@GetMapping("/{id}")
	public ApiResponse<MasterDataAdminService.RecordResponse> get(@PathVariable Long id) {
		return ApiResponse.ok(this.service.get(MasterDataAdminService.Resource.UNIT, id));
	}
	@PostMapping
	public ApiResponse<MasterDataAdminService.RecordResponse> create(@Valid @RequestBody MasterDataAdminService.RecordRequest request,
			@AuthenticationPrincipal CurrentUser currentUser, HttpServletRequest servletRequest) {
		return ApiResponse.ok(this.service.create(MasterDataAdminService.Resource.UNIT, request, currentUser, servletRequest));
	}
	@PutMapping("/{id}")
	public ApiResponse<MasterDataAdminService.RecordResponse> update(@PathVariable Long id,
			@Valid @RequestBody MasterDataAdminService.RecordRequest request,
			@AuthenticationPrincipal CurrentUser currentUser, HttpServletRequest servletRequest) {
		return ApiResponse.ok(this.service.update(MasterDataAdminService.Resource.UNIT, id, request, currentUser, servletRequest));
	}
	@PutMapping("/{id}/enable")
	public ApiResponse<MasterDataAdminService.RecordResponse> enable(@PathVariable Long id,
			@AuthenticationPrincipal CurrentUser currentUser, HttpServletRequest servletRequest) {
		return ApiResponse.ok(this.service.enable(MasterDataAdminService.Resource.UNIT, id, currentUser, servletRequest));
	}
	@PutMapping("/{id}/disable")
	public ApiResponse<MasterDataAdminService.RecordResponse> disable(@PathVariable Long id,
			@AuthenticationPrincipal CurrentUser currentUser, HttpServletRequest servletRequest) {
		return ApiResponse.ok(this.service.disable(MasterDataAdminService.Resource.UNIT, id, currentUser, servletRequest));
	}
}
```

Create the other controllers with the same public method names as `UnitAdminController`, but with explicit resource mapping:

| 文件 | `@RequestMapping` | `MasterDataAdminService.Resource` |
|---|---|---|
| `WarehouseAdminController.java` | `/api/admin/master/warehouses` | `WAREHOUSE` |
| `SupplierAdminController.java` | `/api/admin/master/suppliers` | `SUPPLIER` |
| `CustomerAdminController.java` | `/api/admin/master/customers` | `CUSTOMER` |

Each controller must expose these exact methods and delegate to the service with its own resource enum: `list(keyword,status,page,pageSize)`, `get(id)`, `create(request,currentUser,servletRequest)`, `update(id,request,currentUser,servletRequest)`, `enable(id,currentUser,servletRequest)`, `disable(id,currentUser,servletRequest)`. After creation, run `rg -n "Resource\.(UNIT|WAREHOUSE|SUPPLIER|CUSTOMER)" apps/api/src/main/java/com/qherp/api/system/master` and confirm each resource appears in exactly one controller.

- [ ] **Step 5: 补权限映射**

Modify `PermissionAuthorizationManager.permissionCode`, before permissions tree mapping:
...
### Task 4: 后端物料分类与物料档案接口

**Files:**
- Create: `apps/api/src/main/java/com/qherp/api/system/master/MaterialCategoryAdminService.java`
- Create: `apps/api/src/main/java/com/qherp/api/system/master/MaterialCategoryAdminController.java`
- Create: `apps/api/src/main/java/com/qherp/api/system/master/MaterialAdminService.java`
- Create: `apps/api/src/main/java/com/qherp/api/system/master/MaterialAdminController.java`
- Create: `apps/api/src/test/java/com/qherp/api/system/master/MaterialAdminControllerTests.java`
...
class MaterialAdminControllerTests extends PostgresIntegrationTest {
	...
	private long createCategory(AuthenticatedSession admin, String code, String name, Long parentId) throws Exception {
		ResponseEntity<String> response = exchange(HttpMethod.POST, "/api/admin/master/material-categories",
				Map.of("code", code, "name", name, "parentId", parentId, "status", "ENABLED"), admin);
		assertThat(response.getStatusCode()).isEqualTo(HttpStatus.OK);
		return data(response).get("id").longValue();
	}
...
Expected: FAIL because material controllers do not exist.

- [ ] **Step 3: 实现物料分类服务与控制器**

Create `MaterialCategoryAdminService.java` implementing:
...
Create `MaterialCategoryAdminController.java` with `@RequestMapping("/api/admin/master/material-categories")`. The controller must expose `GET /`, `GET /{id}`, `POST /`, `PUT /{id}`, `PUT /{id}/enable`, `PUT /{id}/disable` and delegate to `MaterialCategoryAdminService`.

- [ ] **Step 4: 实现物料档案服务与控制器**

Create `MaterialAdminService.java` implementing:
...
Create `MaterialAdminController.java` with `@RequestMapping("/api/admin/master/materials")`.
...
### Task 5: 前端 API、路由和通用主数据页面能力

**Files:**
- Create: `apps/web/src/shared/api/masterDataApi.ts`
- Create: `apps/web/src/modules/master/shared/masterPageHelpers.ts`
- Create: `apps/web/src/modules/master/shared/MasterDataTableView.vue`
- Modify: `apps/web/src/router/index.ts`
- Modify: `apps/web/src/App.vue`
- Modify: `apps/web/src/main.ts`
- Create: `apps/web/src/shared/api/masterDataApi.spec.ts`
...
describe('masterDataApi', () => {
  it('按资源路径请求基础资料列表和创建接口', async () => {
    const fetcher = vi.fn()
      ...
      .mockResolvedValueOnce(new Response(JSON.stringify({ success: true, data: { id: 1, code: 'PCS', name: '件', status: 'ENABLED' }, code: 'OK', message: '成功' })))
    const api = createMasterDataApi({ fetcher })

    await api.units.create({ code: 'PCS', name: '件', precisionScale: 0, status: 'ENABLED' })

    expect(fetcher).toHaveBeenLastCalledWith('/api/admin/master/units', expect.objectContaining({
      method: 'POST',
      body: JSON.stringify({ code: 'PCS', name: '件', precisionScale: 0, status: 'ENABLED' }),
    }))
  })
})
```
...
Expected: FAIL because `masterDataApi.ts` does not exist.
...
export interface UnitPayload { code: string; name: string; precisionScale: number; status: MasterDataStatus; sortOrder?: number; remark?: string }
...
export interface CategoryPayload { code: string; name: string; parentId?: string | number | null; status: MasterDataStatus; sortOrder?: number; remark?: string }
...
function createResource<TPayload extends object>(basePath: string, request: <T>(path: string, init: RequestInit) => Promise<T>, get: <T>(path: string, query?: object) => Promise<T>, write: <T>(method: 'POST' | 'PUT', path: string, body?: unknown) => Promise<T>) {
  return {
    list: (query: MasterListQuery) => get<PageResult<MasterRecord>>(basePath, query),
    get: (id: string | number) => get<MasterRecord>(`${basePath}/${encodeURIComponent(String(id))}`),
    create: (payload: TPayload) => write<MasterRecord>('POST', basePath, payload),
    update: (id: string | number, payload: TPayload) => write<MasterRecord>('PUT', `${basePath}/${encodeURIComponent(String(id))}`, payload),
    enable: (id: string | number) => write<MasterRecord>('PUT', `${basePath}/${encodeURIComponent(String(id))}/enable`),
    disable: (id: string | number) => write<MasterRecord>('PUT', `${basePath}/${encodeURIComponent(String(id))}/disable`),
  }
}
...
export function createMasterDataApi(options: Options = {}) {
  ...
  return {
    units: createResource<UnitPayload>('/api/admin/master/units', request, get, write),
    warehouses: createResource<WarehousePayload>('/api/admin/master/warehouses', request, get, write),
    suppliers: createResource<PartnerPayload>('/api/admin/master/suppliers', request, get, write),
    customers: createResource<PartnerPayload>('/api/admin/master/customers', request, get, write),
    categories: createResource<CategoryPayload>('/api/admin/master/material-categories', request, get, write),
    materials: createResource<MaterialPayload>('/api/admin/master/materials', request, get, write),
  }
}
...
Create `MasterDataTableView.vue` as a thin component with slots:
...
Modify `apps/web/src/App.vue`, extend `supportedMenuPaths`:
...
- [ ] **Step 6: 注册新增 Element Plus 组件**

Modify `apps/web/src/main.ts` to add components used by new pages:
...
Create `UnitListView.spec.ts` with:
...
describe('计量单位页', () => {
  beforeEach(() => {
    ...
    apiMock.units.create.mockResolvedValue({ id: 1, code: 'PCS', name: '件', status: 'ENABLED' })
    apiMock.units.update.mockResolvedValue({ id: 1, code: 'PCS', name: '件', status: 'ENABLED' })
    apiMock.units.enable.mockResolvedValue({ id: 1, code: 'PCS', name: '件', status: 'ENABLED' })
    apiMock.units.disable.mockResolvedValue({ id: 1, code: 'PCS', name: '件', status: 'DISABLED' })
  })
  ...
  it('支持查询、创建和无权限隐藏新增按钮', async () => {
    ...
    await flushPromises()
    expect(apiMock.units.create).toHaveBeenCalledWith(expect.objectContaining({ code: 'PCS', name: '件' }))
...
Create `UnitListView.vue` using the same local-state pattern as `UserListView.vue`. Required behavior:
...
const form = reactive({ code: '', name: '', precisionScale: 0, status: 'ENABLED' as MasterDataStatus, remark: '' })
```
...
Submit should call `masterDataApi.units.create()` or `update()`, close dialog on success and reload list. Save failure should display `errorMessage(caught)` and keep dialog open.

- [ ] **Step 4: 复制并调整仓库、供应商、客户页面**

Create `WarehouseListView.vue`, `SupplierListView.vue`, `CustomerListView.vue` by adapting the unit page:
...
Each page needs one focused spec mirroring unit test, verifying create payload and permission hiding.
...
### Task 7: 前端物料分类和物料档案页面

**Files:**
- Create: `apps/web/src/modules/materials/categories/MaterialCategoryView.vue`
- Create: `apps/web/src/modules/materials/categories/MaterialCategoryView.spec.ts`
- Create: `apps/web/src/modules/materials/items/MaterialItemListView.vue`
- Create: `apps/web/src/modules/materials/items/MaterialItemListView.spec.ts`

- [ ] **Step 1: 写物料分类测试**

Create `MaterialCategoryView.spec.ts` with concrete assertions:
...
describe('物料分类页', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    apiMock.categories.list.mockResolvedValue({ items: [{ id: 1, code: 'RAW', name: '原材料', parentId: null, status: 'ENABLED' }], page: 1, pageSize: 100, total: 1 })
    apiMock.categories.create.mockResolvedValue({ id: 2, code: 'SEMI', name: '半成品', parentId: null, status: 'ENABLED' })
  })
  ...
  it('展示分类并提交新增分类', async () => {
    ...
    await flushPromises()
    expect(apiMock.categories.create).toHaveBeenCalledWith(expect.objectContaining({ code: 'SEMI', name: '半成品', parentId: null, status: 'ENABLED' }))
  })
...
Create `MaterialItemListView.spec.ts` with a create-flow test that mocks `materials`, `units`, and `categories`, opens the create dialog, fills code/name/type/source/category/unit/status, submits the form, and asserts this payload:
...
Create `MaterialCategoryView.vue`:

- Load categories with `masterDataApi.categories.list({ page: 1, pageSize: 100, keyword, status })`.
- Build tree locally from `parentId`.
- Show left tree and right table.
- Form fields: code, name, parentId, status, sortOrder, remark.
- Permissions: `master:material-category:create`, `master:material-category:update`.
- Disable action asks `window.confirm('确认停用物料分类“名称”？')`.

- [ ] **Step 5: 实现物料档案页**

Create `MaterialItemListView.vue`:

- Load materials via `masterDataApi.materials.list`.
- Load enabled units and enabled categories for selects.
- Filters: keyword, status, categoryId, materialType, sourceType.
- Form fields: code, name, specification, materialType, sourceType, categoryId, unitId, status, remark.
- Detail drawer shows code, name, specification, categoryName, unitName, materialType label, sourceType label, status.
- Permissions: `master:material:create`, `master:material:update`.
- Save failure keeps form open and restores button state.
...
### Task 8: 本地部署、浏览器验收和视觉分析

**Files:**
- Create: `docs/testing/master-data-material-visual-audit/notes.md`
- Add screenshots under `docs/testing/master-data-material-visual-audit/`
- Modify: `docs/tasks/005-master-data-material-foundation.md`
- Modify: `docs/testing/master-data-material-test-plan.md`
...
Expected: API returns `{"service":"qherp-api","status":"UP"}` and frontend returns HTTP 200.

- [ ] **Step 4: 浏览器功能验收**

Using in-app browser:

1. Login `admin / Qherp@2026!`.
2. Open `/master/units`, create `PCS / 件`.
...
11. Login read-only user or create a master-data-readonly role, verify write buttons hidden and backend write endpoint returns `AUTH_FORBIDDEN`.
...
Append execution record to `docs/tasks/005-master-data-material-foundation.md` and `docs/testing/master-data-material-test-plan.md`:
...
**Files:** no source edits unless validation finds defects.
...
Expected: current feature branch clean.
...
Also summarize automatic tests, browser verification, visual analysis path, and any intentionally out-of-scope items.
