...
创建 `docs/tasks/005-master-data-material-foundation.md` with this structure:
...
创建 `docs/api/master-data-material-api.md` with endpoint tables for:
...
修改 `apps/api/src/main/java/com/qherp/api/system/init/AccountPermissionInitializer.java`, append permission seeds after `system:audit:view`:
...
创建 `apps/api/src/test/java/com/qherp/api/system/master/MasterDataAdminControllerTests.java` with tests:
...
public class MasterDataAdminService {
		...
		private Object[] insertArgs(MasterDataRequest request, MasterDataStatus status, String operator, OffsetDateTime now) {
			return switch (this) {
				case UNIT -> {
					...
					yield new Object[] { r.code(), r.name(), r.precisionScaleOrDefault(), r.sortOrderOrDefault(),
	...
	public record UnitRequest(@NotBlank String code, @NotBlank String name, Integer precisionScale, Integer sortOrder,
		...
		int precisionScaleOrDefault() {
			return precisionScale == null ? 0 : precisionScale;
		}

		int sortOrderOrDefault() {
...
创建 the other controllers with the same public method names as `UnitAdminController`, but with explicit resource mapping:
...
创建 `MaterialCategoryAdminService.java` implementing:
...
创建 `MaterialCategoryAdminController.java` with `@RequestMapping("/api/admin/master/material-categories")`. The controller must expose `GET /`, `GET /{id}`, `POST /`, `PUT /{id}`, `PUT /{id}/enable`, `PUT /{id}/disable` and delegate to `MaterialCategoryAdminService`.

- [ ] **步骤 4：实现物料档案服务与控制器**

创建 `MaterialAdminService.java` implementing:
...
创建 `MaterialAdminController.java` with `@RequestMapping("/api/admin/master/materials")`.
...
创建 `MasterDataTableView.vue` as a thin component with slots:
...
- 展示左侧分类树和右侧表格。
- 表单字段：code、name、parentId、status、sortOrder、remark。
...
6. 打开 `/materials/categories`，创建 `RAW / 原材料`、`SEMI / 半成品`、`FIN / 成品`、`AUX / 辅料`。
