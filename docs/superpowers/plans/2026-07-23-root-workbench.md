...
- [x] **步骤 2：汇总评审意见并处理阻断与严重问题**

每个需要修改的代码问题都先添加或调整失败测试，确认失败后再修复。一般问题登记后续清单，不反复阻断。

- [x] **步骤 3：请固定测试角色只复审修复差异**

只有差异复审仍存在阻断或严重问题时才继续返工。

评审结果：

- 固定测试角色首轮评审为阻断 `0`、严重 `0`、一般 `1`，一般项是多条同类型来源受限异常可能生成重复的 Vue 列表键。
- 主代理先增加失败测试，确认实现前因 `exceptionRowKey` 缺失失败，再加入稳定键辅助函数与索引兜底；修复提交为 `f05e2d90e04244be130fecef6d591c871e848228`。
- 同一固定测试角色只复审修复差异，受影响测试 `10/10`、类型检查和 `git diff --check` 通过，最终阻断 `0`、严重 `0`、一般 `0`，允许交付。

- [x] **步骤 4：更新交接、提交、合入并推送**
...
- `codex/040-root-workbench` 已纯快进合入并推送 `main`，正式工作区两份用户未提交文档保持未暂存且未纳入 040。
- 正式 Web 已从最终主分支重建，`http://127.0.0.1:45173/` 和 `/api/health` 均为 200，Flyway V36，数据库/MinIO 42/42，超级管理员真实登录通过。
- 正式 `1280×720` 根路径展示五个常用入口和全部五段工作台结构，无横向溢出、按钮裁切、英文枚举、失败请求或控制台错误/警告，页面留给用户直接验收。
