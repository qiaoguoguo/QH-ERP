...
- 当前状态：021 原阶段已完成并进入 `main`；2026-07-13 发现的 BOM 时效状态表达及生产工单按计划开工日期消费 BOM 的阻断缺陷已完成修复、集中审查、整改、差异复审、交付前自动化验证、桌面浏览器验收和视觉复验，并已通过纯快进合入 `main`，随最终交接记录推送至 `origin/main`。
- 当前分支：`main`；修复来源分支为 `codex/021-bom-effective-date-fix`，由 Codex 宿主工作树继续保留。
...
- 本次修复基线：受影响前端测试 27/27、受影响后端测试 22/22 通过；后端命令固定使用 JDK 21。
- 本次修复最终验证：后端 323/323；前端 83 个测试文件、663/663；类型检查、生产构建和 `git diff --check` 通过；真实桌面浏览器功能验收与视觉差异复验均为 `APPROVED`。本次无迁移且未新增迁移，按本文件“集中审查与验证”约束不重复空库和 V17 存量升级，继续引用原 021 已通过的 V1-V18 与 V17→V18 证据。
...
### 2026-07-13：BOM 时效缺陷修复交付结果

- 实现提交：`3afb748 修复BOM时效与工单有效期约束`，完成 BOM 双状态、工单按 `plannedStartDate` 查询和创建/更新/发布三入口强校验。
- 集中整改提交：`059e901 完善BOM时效竞态与回归保护`，修复候选与明细异步旧响应覆盖、非草稿历史工单回显保护、加载中保存门禁、候选归属校验和默认历史筛选参数，并补齐真实候选过滤、错误父项、双空边界和已发布快照不回写测试。
- 视觉整改提交：`44ffb29 优化BOM列表时效状态可见性`，将发布状态、时效状态和有效期前移到 BOM 编码后；默认 1280 桌面视口无需横向滚动即可扫描三组核心状态，操作列继续固定右侧，较窄视口仍保留横向滚动结构。
- 集中审查：后端实现无阻断或严重问题；前端首次审查发现异步竞态和非草稿历史回显两项严重问题，测试审查发现真实候选接口、快照保护和错误父项证据缺口，已一次性整改。差异复审后阻断与严重问题为零。
- 交付窗口：首次视觉检验因 BOM 列表核心双状态不在 1280 首屏而 `REJECTED`；定向修复、交叉代码复核、前端全量重验和真实页面差异复验后转为 `APPROVED`。
- 自动化结果：后端全量 323/323；前端最终 83 个测试文件、663/663；`npm run typecheck`、`npm run build` 和 `git diff --check` 退出码均为 0，Vite 构建为 2007 个模块。
- 浏览器结果：`021-BROWSER` 同父项 V1/V2 同时保留，分别显示“当前有效/未来生效”；工单 2026-07-13 只候选 V1，2026-08-01 只候选 V2，2026-06-30 显示无有效 BOM；日期变化会清理旧选择和明细；既有已发布工单仍显示原 BOM 和用料快照。未保存新工单，控制台无错误或警告。
- 验收边界：只做本次相关页面的桌面端验收和视觉检验；设计开发继续保留横向滚动等响应式结构；未测试移动端、窄屏或触屏，未采集或保存截图。
- 环境边界：PostgreSQL 与后端 Testcontainers 临时数据库运行在 Docker；前端、后端服务及 Node/Maven 验证运行在 Windows 宿主机，不能表述为开发、测试和验收全部容器化。
- 后续一般项：编辑草稿时若 BOM 候选接口失败而原 BOM 明细加载成功，候选失败提示仍可能被后续明细成功状态覆盖；生产工单长物料/单位文本在极长验收编码下可能截断。两项均不影响正常候选、后端强校验、历史快照或本次桌面验收路径，按阶段规则进入后续清单，不再开启审查循环。
