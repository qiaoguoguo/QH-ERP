...
- 阶段编号：`024`
- 阶段名称：`采购深化与项目专采`
- 当前状态：权威阶段说明已完成交叉审核整改并冻结，整包开发尚未开始。
- 权威说明基线：`detached HEAD@8582536939d274ca9e9d07a45a463be1ce2eda9f`。
...
- 本文件冻结后，主代理按本文向固定五角色派发实现、集中审查、整改、差异复审和交付前验证任务。
...
- 当前筛选导出任务类型至少包含 `PROCUREMENT_REQUISITION_EXPORT`、`PROCUREMENT_INQUIRY_EXPORT`、`PROCUREMENT_QUOTE_EXPORT`、`PROCUREMENT_PRICE_AGREEMENT_EXPORT`、`PROCUREMENT_ORDER_EXPORT`、`PROCUREMENT_SUPPLY_EXPORT`。
...
| 采购文档任务 | `/api/admin/platform/document-tasks` | 复用 022 任务创建、确认、下载和取消 |
...
- 整包开发与定向验证：尚未开始。启动后按本文工作包和开发期定向验证执行；本次整改未启动业务实现。
- 阶段集中审查：尚未开始。功能完整后由固定五角色执行唯一一轮集中审查。
- 集中整改与差异复审：尚未开始。阻断和严重问题一次性派回对应角色整改，只复审差异。
- 唯一交付前全量验证：尚未开始。集中复审通过后统一执行全量后端、全量前端、类型检查、生产构建、迁移、真实桌面浏览器功能与视觉检验、环境健康和空白检查。
- 变更：本次冻结不启动 025/026，不编辑其他阶段说明，不改变 000 至 023 已交付事实。
- 桌面浏览器与视觉结论：尚未执行；024 实现完成后只检验本阶段新增或修改的桌面页面，不截图、不验收移动端。
- 合入、推送和主分支部署：尚未执行；全量验证通过后按项目规则更新交接、合入、推送并从主分支启动验收服务。
- 结论：024 已具备进入整包开发的唯一权威阶段说明；实现和验收必须以本文为边界。
