...
- 2026-07-18 用户明确指出 027 有较多页面未遵循 `docs/ui/page-standards.md`。此前浏览器验证只证明已覆盖路径的业务功能、关键状态、控制台和明显溢出满足当时检查范围，不构成页面规范逐项验收；027 页面规范治理尚未闭合。
...
- 结论：027 业务范围实现、集中审查整改、全量验证、主分支部署和已识别严重视觉差异复验已经完成；页面规范专项核对与整改尚未完成，不能宣称 027 页面规范已闭合。下一业务阶段仍为尚未启动的 028，但不得在 028 中复制 027 的不合规页面模式。

## 页面规范治理遗留

- 当前尚未形成 027 全部新增、修改和受影响页面到 `docs/ui/page-standards.md` 检查项的逐页差异清单，不得猜测具体未合规页面数量或把未检查页面写成已通过。
- 后续页面治理必须先由固定 UI 设计师基于真实运行页面完成全量清点，再由产品经理确认业务优先级、前端开发整改、测试独立复验；主代理只负责汇总和范围控制。
- 整改至少覆盖页面骨架、标题与说明、查询栏、表格与分页、表单、候选池、弹窗与抽屉、操作按钮、权限/异常/空态及桌面端实际可达性。每个页面必须留下适用检查项、问题、整改和结论。
- 该遗留项不得被静默并入 028 发票、费用与往来业务范围；如决定先做专项治理，应在用户确认后建立独立、唯一的治理说明。无论是否先行整改，028 及后续新增页面从设计开始就必须完整遵循 `page-standards.md`。
