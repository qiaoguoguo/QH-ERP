...
- 状态：整包开发、集中审查、全量验证窗口、集中整改、最终差异复验、主分支合入推送及正式 V31 部署均已完成；029 已正式交付。
- 冻结日期：2026-07-19。
- 五角色目标讨论：已完成唯一一轮。
...
- 2026-07-19：全量窗口集中整改完成。历史迁移 9 类加 V31 共 `28/28` 通过；无金额后端定向 `17/17` 通过；前端整改定向 `17/17`、类型检查和生产构建通过；演示生成器静态自测通过。新的正式 V30 副本只通过真实 API 增量建立 029 最小链路，首次与幂等复跑均成功，材料 `168`、人工 `300`、外协 `120`、项目费用 `200`、制造费用 `50`，项目总成本 `838`、发货收入 `10000`、发货经营毛利 `9162`，手工人工仅一条 `300`，公共材料 `110` 保持 `PUBLIC` 且不写项目层。
- 2026-07-19：最终受影响路径复验证明无金额账号 7 个接口均为 HTTP 200 且金额字段为 `null`，无 `0` 冒充、无 500、无泄露；029 重算和确认阻断前后对采购、库存、生产、销售、费用共 106 个上游对象做稳定指纹，差异为 0；确认继续以 `PROJECT_COST_LABOR_UNPRICED` 返回 409。真实 Chromium `1280×720` 覆盖 8 个受影响入口，页面失败、控制台错误、页面异常和资源失败均为 0；UI 逐页复验阻断/严重为 0，仅保留已确认调整只读页“取消/提交提示”语义一般项。
- 2026-07-19：产品最终复验发现刚重算运行仍显示 `STALE`。根因是查询层把 `freshnessStatus` 错误等同 `isCurrent`，且来源指纹遗漏三种收入、有效 latest 可能选择取消运行、当前性筛选存在未分页高代价完整映射。原后端角色按 TDD 修复：新鲜度改为完整来源与发货/开票/目标收入指纹比较；有效 latest 统一为活动运行、当前确认、非取消历史；筛选改为轻量候选后仅映射当前页。交叉差异复审确认原 1 个阻断、2 个严重全部闭合，后端受影响 `20/20` 通过。
- 2026-07-19：提交 `b637abe4ad70fe4b434e8bac3773d2dfaefe2c46` 的最终隔离复验证明工作台、项目详情和运行详情均为 `CURRENT`，`calculationId/latestCalculationId` 一致，未确认运行 `isCurrent=false` 的历史语义保持；838/10000/9162、未定价确认阻断、106 个上游对象不回写、无金额权限和桌面页面均无回归。产品经理与 UI 设计师分别同意进入交付；阻断、严重均为 0。
- 2026-07-19：全部 029 临时端口、6 个临时数据库、6 个临时 bucket、8 个仓库外临时目录与凭据已统一清理为 0。正式 `18080/5173`、V30、V29/V30 checksum、失败迁移 0 和数据库/MinIO `18/18` 保持不变；功能分支工作树干净，进入主分支合入与正式部署。
- 2026-07-19：029 业务交付提交 `691f929fe89b23ebcc45fe4c56e92d5f01db51a6` 已从功能分支纯快进合入并推送 `main`；合入后后端定向 20/20、前端定向 17/17、生成器自测与差异检查均再次通过。
- 2026-07-19：正式部署前备份位于 `C:\Users\14567\.codex\backups\qherp\029-main-before-v31-20260719-190016`，PostgreSQL dump 928869 字节、可读恢复清单 141435 字节、MinIO 18 个对象共 44385 字节。正式数据库由 V30 自然前迁 V31，V29/V30/V31 checksum 分别为 `774334682`、`2130342893`、`-2074547591`，失败迁移 0；独立验证 117/117，数据库与 MinIO 18/18。
- 2026-07-19：正式主分支 API `http://127.0.0.1:18080` 健康为 `UP`，Web `http://127.0.0.1:5173` 返回 200；真实 Chromium `1280×720` 核对项目成本工作台和项目详情，页面错误、控制台错误、资源失败和页面级溢出均为 0。部署时未为展示补造 029 业务事实；随后管理员对既有项目 1 合法执行一次核算和一次重算，当前保留 1 条 `CALCULATED` 运行、1 条 `DELIVERY_WITHOUT_FINISHED_COST/BLOCKING/OPEN` 差异和 2 条动作幂等记录，来源行、成本分录及调整均为 0，审计动作成功。该真实验收运行不影响阻断与严重问题为 0 的交付结论，029 正式交付完成。
