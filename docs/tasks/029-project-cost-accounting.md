...
- 2026-07-19：正式主分支 API `http://127.0.0.1:18080` 健康为 `UP`，Web `http://127.0.0.1:5173` 返回 200；真实 Chromium `1280×720` 核对项目成本工作台和项目详情，页面错误、控制台错误、资源失败和页面级溢出均为 0。正式库没有为展示补造 029 业务事实，核算运行、调整及动作幂等业务行保持 0；阻断与严重问题为 0，029 正式交付完成。
