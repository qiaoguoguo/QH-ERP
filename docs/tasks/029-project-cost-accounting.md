...
- 2026-07-19：唯一交付前全量验证窗口已按计划完整执行并统一汇总。前端全量测试 `114/114` 文件、`995/995` 用例通过；V1→V31、V30→V31、正式 V30 副本、精确 checksum、验证器 `117/117`、资源 `18/18`、真实公共费用调整/022 审批/幂等/冲突/来源变化 409 和 029 不回写上游均取得证据。窗口同时发现：9 个历史迁移回归类共 24 个断言仍把最新版本写死为 V30；前端 3 个类型错误导致 typecheck/build 失败；无金额账号因 `inventoryValuationSource(null)` 返回 500；已确认调整仍可进入可保存的编辑表单；正式副本缺少通过真实 API 建立的 838/9162 最小数据。主代理据此判定当前不得交付，进入全量窗口唯一一次集中整改。
- 2026-07-19：全量窗口整改按职责一次性回派：测试角色修正历史迁移回归期望并扩展真实 API 演示数据生成器；后端角色修复无金额查询 NPE；前端角色修复类型/构建与已确认调整只读态。整改不得修改 V1-V31、不得用 SQL 直灌业务事实、不得重跑未受影响全量集合；完成后只复验失败差异和受影响路径，并在新的正式 V30 副本隔离环境中证明 838/9162、权限态、已确认只读态及上游不回写。
