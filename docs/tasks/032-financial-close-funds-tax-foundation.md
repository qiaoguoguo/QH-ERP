...
- 状态：032 已完成整包开发、审查整改、唯一全量窗口、产品文档、主分支合入推送、正式 V34 自然前迁、正式验证和隔离资源清理；最终阻断和严重问题为 0，正式服务可供用户验收。
- 冻结日期：2026-07-20。
...
| 2026-07-21 | 按集中审查结论完成一次集中整改并重新冻结 V34 | 五角色集中审查识别出结账九项门禁、系统来源凭证唯一与新鲜度、敏感字段脱敏、银行方向化未达、页面动作闭环及对应契约和测试缺口；整改同时调整了 V34 权限、科目、约束和测试数据门禁。 | 后端、前端、测试原固定角色按一次性去重清单完成整改；税务汇总失效沿用来源指纹派生 `stale/current` 与新版本机制，不新增独立失效端点。差异复审只覆盖本轮整改差异和受影响路径，最终 checksum 以后续必要返工冻结值为准。 |
| 2026-07-21 | 闭合差异复审识别出的阻断和严重问题 | 五角色差异复审确认核心后端规则已闭合，但发现税额汇总动作能力、税务写幂等、追溯 DTO、银行流水菜单、银行候选/多对多/科目主键、日期控件和受影响分页仍影响真实桌面验收。 | 只将去重后的阻断和严重问题派回原后端、前端、测试角色；不重审未变化阶段。V34 增补银行流水路由种子后 checksum 最终冻结为 `-629066235`，独立验收与验证器同步更新；完成修复差异最终复核后方可进入全量验证窗口。 |
| 2026-07-21 | 闭合唯一全量验证窗口的历史迁移回归门禁 | 全量后端 716 项中有 30 项失败，全部来自 12 个历史迁移回归类仍把最新成功 Flyway 版本断言为 V33；其余全量、构建、迁移、验证器、桌面和环境项目均已继续执行并通过。 | 只同步 12 个历史迁移回归类的最新版本断言至 V34，保留并补齐 V33/V34 精确 checksum；后端角色定向验证和测试角色独立差异复验均为 32/32 与 V34 2/2 通过，不重新运行已完成的全量窗口。 |
| 2026-07-21 | 同步正式交付前产品与操作文档 | 长期产品文档仍把 032 记作未来阶段，且缺少财务关闭、银行和税务基础的用户操作口径。 | 产品固定角色只更新项目入口、文档索引、流程、决策、数据词汇、演示数据、操作手册和工具说明；准确记录 V34/170 交付前验证事实，同时明确正式环境尚未部署 V34。 |
| 2026-07-21 | 修正正式零业务事实验证器契约 | 正式 V34 自然前迁后无 032 业务事实，完整验证器的两条隔离样本存在性规则因此得到 168/170；迁移、对象、页面和正式事实均正常。 | 不补造正式数据、不按数据库名特判；把失败样本和税款幂等规则改为“零事实合法、有事实则完整且唯一”，自测及正式/隔离验证器均 170/170。 |
| 2026-07-21 | 完成正式 V34 部署与交接 | 全量门禁、正式备份、主分支与远端同步、V34 迁移、正式验证和桌面检查全部闭合。 | 保留正式零业务事实；清理隔离端口、数据库、bucket 和临时目录；032 完成，下一阶段 033 未启动。 |
...
- 测试工作包：集中整改前 `FinancialCloseStage032AcceptanceTests` 5 项和完整验证器 `157/157` 通过；集中整改后独立验收扩展为 10 项并全部通过，差异复审必要返工后完整验证器最终扩展为 `170/170` 通过。演示数据验证器自测和 032 隔离策略默认路径通过，正式 `qherp/qherp-private` 资源拒绝路径按预期失败关闭；V34 checksum 最终精确冻结为 `-629066235`。隔离 API `18083`、Web `5175`、数据库 `qherp_032_review` 与对象桶 `qherp-032-review` 保持可用；正式 API `18080`、Web `5173`、V33 数据库和对象桶状态未变化。
...
- 审查时运行证据：隔离 API `18083`、Web `5175`、当时 V34 checksum 和完整验证器 `157/157` 均已核验；9 个新增页面和主要受影响桌面页面可登录渲染，正式 `18080/5173` 与 V33 资源未变化。这些证据只证明环境可审查，不抵消下列业务、权限、契约和页面缺陷；必要返工后的最终 checksum 为 `-629066235`。
- 主代理合并去重后的阻断簇：
...
- 测试整改独立证据：`FinancialCloseStage032AcceptanceTests` 10/10、`demo-data-self-test.ps1` 和集中整改时完整验证器均通过；必要返工后完整验证器最终为 `170/170`，V34 checksum `-629066235`，失败迁移 0；隔离数据库与对象桶对象一致性 18/18。
...
- 五角色差异复审基线为 `c97b06d49325f571d33d4d695822322c3630ebe7`，原始报告位于 `.superpowers/sdd/032-diff-review-*.md`。主代理合并为 1 个阻断簇和严重问题簇：税额汇总缺行级动作能力；税务写动作幂等和银行流水菜单种子不完整；结账与 031 会计期间追溯 DTO 不匹配；银行未达仍可默认候选、多对多表达不足、账户科目主键错误；日期控件和损益分页不符合页面规范。
- 必要返工后端证据：后端自有红测先出现 3 项失败、2 项错误，随后税务/DTO/V34 组合 11/11、财务关闭服务 3/3 和生产代码编译通过；税额汇总动作、税务幂等、追溯字段、银行账户主键与 `/gl/bank-statements` 种子已闭合。
- 必要返工前端证据：红测先出现 4 个测试文件失败、8 项失败；实现后收口定向测试 5 文件 31 项、类型检查和前端空白检查通过；显式候选、可编辑匹配明细、真实科目 ID、日期选择器、追溯归一化和分页已闭合。
- 必要返工测试证据：独立验收 10/10、前端受影响复核 7 文件 25/25、类型检查、`demo-data-self-test.ps1` 和完整验证器 `170/170` 通过；隔离资源从正式 V33 只读克隆后自然迁移 V34，未执行 `repair` 或手改 Flyway 历史，正式 V33 和对象 18/18 未变化。
- 五角色最终差异复核已完成：产品核对验收矩阵，UI 检查真实桌面页面，前端与后端交叉审查非本人实现，测试核对覆盖与回归风险；阻断和严重问题均为 0，结论一致允许进入唯一交付前全量验证窗口。原始报告位于 `.superpowers/sdd/032-final-recheck-*.md`。
- 差异复核登记的一般后续项：检查详情的检查项编码列显示 `-`；少量状态、税种和银行未达枚举仍显示原始编码；已确认银行对账的候选复选框仍可产生本地勾选状态；部分密集表格长文本依赖 tooltip；生产构建保留分块体积提示。这些问题不影响数据、权限、状态、关键动作或当前桌面验收，不重复阻断 032 交付。
- 当前门禁：差异复审、最终差异复核、唯一交付前全量验证、正式部署验证及缺陷差异复验均已通过；正式交付已完成。

### 唯一交付前全量验证窗口

- 测试固定角色在 `9ca7ac8e8f8c3ffaa676283ab1c1fb2b2d84093c` 基线上一次性执行全部计划内可执行项目。全量前端 124 个文件、1067 项测试通过；类型检查和生产构建通过；V34 双路径迁移回归 2/2、演示数据自测、隔离策略和完整验证器 `170/170` 通过；隔离 V34 checksum `-629066235`、失败迁移 0、数据库与 MinIO 对象 18/18。生产构建仅有既有分块体积提示。
- 全量后端首次执行为 716 项、30 失败、0 错误、0 跳过。全部 30 个失败集中在 12 个 V22 至 V33 历史迁移回归类仍断言最新成功版本为 V33，不涉及 032 生产代码、V34 迁移事实或业务验收。窗口未因此中断，其余项目均继续执行并统一记录。
- 后端固定角色只修改上述 12 个历史迁移回归类，把最新成功版本同步为 V34，并保留/补齐 V33 `612501943`、V34 `-629066235` 精确 checksum 和既有历史事实断言；定向组合 32/32、V34 专项 2/2 通过。测试固定角色独立审查同一差异并再次得到 32/32、2/2、退出码 0，结论 `FULL_VALIDATION_REMEDIATION_PASS`。按全量窗口规则只复验缺陷差异和受影响路径，不重复执行已完成的全量项目。
- 真实桌面浏览器检查覆盖 12 个新增或受影响页面，页面均可访问、加载完成、页面级横向溢出 0、控制台错误 0；管理员、有限权限和无动作账号的动作禁用、路由守卫及银行/税务敏感字段脱敏符合验收矩阵。未截图、未测试移动端，符合阶段边界。
- 隔离 API `18083`、Web `5175`、数据库 `qherp_032_review` 和 bucket `qherp-032-review` 保持健康；正式 API `18080`、Web `5173`、V33 数据库和正式对象 18/18 在窗口前后未变化，未迁移、未写入、未停止正式资源。
- 最终结论：唯一交付前全量验证窗口通过；阻断和严重问题为 0。一般后续项沿用差异复核登记，不为一般问题重开阶段审查。

### 正式交付

- 交付序列从 `origin/main@30ad38c0948cae2f1d7a64c1f41e4b7c625a750e` 纯快进合入并推送 `main`；032 业务交付候选节点为 `8bc74397f815c5e8b9a1405a0c00ce8e8ed81432`，正式验证器零事实契约修正节点为 `c823861fd467b78119fb964a2f1d41314129186f`。修正后本地 `main`、`origin/main` 与实际远端一致，正式工作树干净。
- 部署前备份位于 `C:\Users\14567\.codex\backups\qherp\032-main-before-v34-20260721-043029`：PostgreSQL custom dump 1,118,966 字节且 `pg_restore -l` 可读，`qherp-private` 镜像 18 个对象、44,385 字节。
- 正式服务从 `F:\zhangqiao\AI-study\qherp` 启动：API 监听 PID `17068`、启动器 PID `616`，Web 监听 PID `14404`、启动器 PID `17476`；`http://127.0.0.1:18080/api/health` 与 `http://127.0.0.1:5173/` 均为 HTTP 200，PostgreSQL 与 MinIO 健康。
- 启动日志显示正式数据库从 V33 自然应用唯一 V34 迁移，成功验证 34 条迁移；V29—V34 checksum 为 `774334682/2130342893/-2074547591/249406902/612501943/-629066235`，失败迁移 0，未执行 `repair` 或手工修改 Flyway 历史。
- 正式完整验证器首次执行的 2 条失败只来自把隔离代表样本存在性误作正式零事实门禁；测试角色以红测把规则改为零事实合法、有事实则完整且唯一，`demo-data-self-test.ps1`、正式 `qherp/qherp-private` 和隔离环境均最终为 `170/170`。数据库 `AVAILABLE` 文件与 MinIO 对象保持 18/18。
- 正式事实保持：030 的 7 月期间仍 `OPEN`，最近检查 `BLOCKED`、3 阻断/2 警告，无关闭、重开或快照；031 凭证、记账、账簿分录和来源占用为 0；032 财务关闭、反结账、银行对账、税务汇总、税款缴纳和幂等业务事实均为 0。V34 税率、票种等配置种子不解释为业务数据。
- 测试固定角色在正式桌面检查 12 个新增或受影响入口：全部可访问、加载收口、控制台错误 0、页面级横向溢出 0；未截图、未测试移动端、未点击写动作。正式库没有专用低权限账号，未为验收新建账号或授权；低权限登录和脱敏动态路径沿用隔离全量窗口已通过证据，正式权限种子只读核对一致。
- 隔离 `18083/5175`、`qherp_032_review`、`qherp-032-review` 和 `C:\Users\14567\.codex\tmp\qherp-032-review` 已清理为 0；正式服务、正式数据库、对象桶和部署前备份未受清理影响。
- 产品与操作文档已由产品固定角色同步至正式 V34 事实。正式交付结论：032 已完成，阻断和严重问题为 0；一般项为少量原始枚举、检查编码列 `-`、确认对账候选本地勾选、tooltip 截断和构建分块提示，不影响数据、权限、状态或关键动作，不重开阶段审查。
