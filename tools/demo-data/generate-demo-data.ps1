...
function Ensure-PurchaseOrder {
    param([string] $Key, $Supplier, [object[]] $Lines)
    $remark = "验收演示采购订单 $Key"
    $existing = Get-FirstByRemark -Path "/api/admin/procurement/orders" -Remark $remark
    if ($null -eq $existing) {
        $existing = Invoke-DemoApi -Session $Session -Method Post -Path "/api/admin/procurement/orders" -Body ([ordered]@{
...
$approvalRole = Ensure-Role -Code "$DemoPrefix-ROLE-APPROVAL" -Name "桥合审批角色" -PermissionCodes ($allPermissionCodes | Where-Object { $_ -like "platform:*" -or $_ -like "sales:contract:*" -or $_ -like "material:bom-eco:*" -or $_ -like "inventory:*" }) -PermissionMap $permissionMap
